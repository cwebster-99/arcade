...
const SnakeGame = () => {
  ...
  const [food, setFood] = useState<Cell>(() =>
    randomFood(initialSnake(), size, size),
  );
    ...
    >
      ...
      <div className="w-full max-w-md mb-4">
        ...
        <label className="block text-xs text-gray-600 mt-2">
          Speed (ms per move): {speed} — {Math.round((1000 / speed) * 10) / 10}{" "}
          moves/sec
        </label>
        <input
          type="range"
            ...
            Array.from({ length: size }).map((__, c) => {
              ...
              return (
                <div
                  key={`${r}-${c}`}
                  role={roleAttr}
                  aria-label={isFood ? "Food" : undefined}
